...
To receive an existing registry instead, use `bun run relay join REGISTRY_KEY` and exchange device trust with its publisher; see the [CLI guide](docs/artifact-cli.md). If your registry already lives in a custom directory, pass `--store PATH` to each command instead of initializing a new default registry.
